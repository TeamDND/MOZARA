...
class PineconeManager:
    ...
    def get_reference_images_by_stage(self, predicted_stage: int, gender: str = None, top_k: int = 3, timeout: float = 3.0) -> Dict[str, str]:
        """
        ...
        Args:
            ...
            top_k: viewpoint별로 가져올 이미지 수 (기본값: 3)
            timeout: 타임아웃 시간(초) (기본값: 3.0)
        ...
        try:
            ...
            for view_key, view_values in viewpoints.items():
                ...
                if time.time() - start_time > timeout:
                    self.logger.warning(f"Reference image search timed out after {timeout}s")
                    break
                ...
                # 증강 파일 제외
                search_filter["is_augmented"] = {"$ne": True}
            ...
            self.logger.warning(f"Reference image search timed out: {e}")
            return reference_images
        except Exception as e:
