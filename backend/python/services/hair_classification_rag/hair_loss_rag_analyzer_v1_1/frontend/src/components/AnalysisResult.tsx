...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ result }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-8 p-6 bg-white shadow-md rounded-lg">
      ...
      {/* 비슷한 사례 이미지 */}
      {(result.reference_image_front || result.reference_image_side) && (
        <div className="mb-6">
          <h3 className="text-xl font-semibold text-gray-700 mb-3">비슷한 사례 이미지</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {result.reference_image_front && (
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-gray-600 mb-2 font-medium">정면</p>
                <img
                  src={`http://localhost:8000/api/images/${encodeURIComponent(result.reference_image_front)}`}
                  alt="정면 참고 이미지"
            ...
            {result.reference_image_side && (
              <div className="bg-gray-50 p-4 rounded-lg">
                <p className="text-sm text-gray-600 mb-2 font-medium">측면</p>
                <img
                  src={`http://localhost:8000/api/images/${encodeURIComponent(result.reference_image_side)}`}
                  alt="측면 참고 이미지"
                  className="w-full h-auto rounded-md border border-gray-200"
                  onError={(e) => {
                    e.currentTarget.style.display = 'none';
