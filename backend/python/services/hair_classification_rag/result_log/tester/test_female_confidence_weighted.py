#!/usr/bin/env python3
r"""
Female Hair Loss - Confidence-Weighted Ensemble (Stage 1-5)
신뢰도 기반 동적 가중치 앙상블 테스트

테스트 대상: C:\Users\301\Desktop\female_classification\test\selected_test (stage_1..stage_5)
방식: 신뢰도 기반 동적 가중치 (per-class 가중치 최적화 불필요)

로그 출력: result_log/log/female_confidence_test{N}/
"""
...
def predict_both(pc: Pinecone, idx_conv, idx_vit, img_path: Path, vit, conv, tf_v, tf_c) -> Tuple[Dict, Dict]:
    """
    양쪽 인덱스에서 검색하고 타이밍 정보 반환
    ...
    timings['vit_embed'] = time.perf_counter() - t0

    # ConvNeXt Embedding
    t0 = time.perf_counter()
    cq = embed(img, conv, tf_c)
    timings['conv_embed'] = time.perf_counter() - t0

    # ViT Search (gender=female 필터)
    t0 = time.perf_counter()
    r_v = idx_vit.query(
        vector=vq.tolist(),
        top_k=TOP_K,
    ...
    timings['vit_search'] = time.perf_counter() - t0

    # ConvNeXt Search (gender=female 필터)
    t0 = time.perf_counter()
    r_c = idx_conv.query(
        vector=cq.tolist(),
        top_k=TOP_K,
    ...
    timings['conv_search'] = time.perf_counter() - t0
...
def main():
    print("="*80)
    print("Female Hair Loss - Confidence-Weighted Ensemble Test (Stage 1-5)")
    print("="*80)
    ...
    for fp, st in test_items:
        try:
            # 예측 및 타이밍
            timings, (p_c, p_v) = predict_both(pc, idx_conv, idx_vit, fp, vit, conv, tf_v, tf_c)
    ...
    with open(log_dir / "report.txt", "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write(f"Female Hair Loss - Confidence-Weighted Ensemble Test - Test #{test_no}\n")
        f.write("=" * 80 + "\n")
        ...
        f.write("-" * 40 + "\n")
        f.write(f"{'Step':<20} {'Mean':<10} {'Std':<10} {'Min':<10} {'Max':<10}\n")
        f.write("-" * 40 + "\n")

        step_names = {
            'image_load': 'Image Loading',
            'vit_embed': 'ViT Embedding',
            'conv_embed': 'ConvNeXt Embedding',
            'vit_search': 'ViT Search',
            'conv_search': 'ConvNeXt Search',
        ...
        for key, name in step_names.items():
            if key in timing_stats:
                ...
                f.write(f"{name:<20} {stats['mean']:<10.2f} {stats['std']:<10.2f} {stats['min']:<10.2f} {stats['max']:<10.2f}\n")

        # 전체 파이프라인 시간
        total_pipeline_mean = sum(timing_stats[k]['mean'] for k in step_names.keys() if k in timing_stats)
        f.write("-" * 40 + "\n")
        f.write(f"{'Total Pipeline':<20} {total_pipeline_mean:<10.2f}\n")
        f.write("\n")
    ...
    plt.title(f'Female Confidence-Weighted Ensemble Confusion Matrix\nTest #{test_no}')
    ...
    bars = plt.bar(names, vals, color=['#9b59b6', '#e74c3c', '#3498db', '#2ecc71'])
    plt.ylim(0, 1)
    for b, v in zip(bars, vals):
        plt.text(b.get_x() + b.get_width() / 2, v + 0.01, f"{v:.3f}", ha='center', va='bottom')
    plt.title(f'Female Confidence-Weighted Ensemble Performance\nTest #{test_no}')
    plt.tight_layout()
    ...
    bars = plt.bar(range(len(step_labels)), step_means, yerr=step_stds, capsize=5,
                   color=['#3498db', '#e74c3c', '#f39c12', '#9b59b6', '#1abc9c', '#34495e', '#95a5a6'])
    plt.xticks(range(len(step_labels)), step_labels, rotation=45, ha='right')
    plt.ylabel('Time (ms)')
    plt.title(f'Pipeline Step Timing Analysis\nTest #{test_no}')
    ...
    print(f"\n{'=' * 80}")
    print(f"Female Confidence-Weighted Ensemble Test 완료! - Test #{test_no}")
    print(f"{'=' * 80}")
