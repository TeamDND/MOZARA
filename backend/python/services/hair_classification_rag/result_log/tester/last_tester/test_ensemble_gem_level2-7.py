#!/usr/bin/env python3
r"""
ConvNeXt + ViT-S/16 앙상블 with GeM Pooling (Level 2-7 전용)
Per-class 가중 소프트보팅 + 선택적 오버라이드 + GeM Pooling

테스트 대상: C:\Users\301\Desktop\classification_test (level_2..level_7)
가중치 로드: 기존 저장된 ensemble_config.json 사용
풀링 방식: GeM (Generalized Mean) Pooling (p=3)

로그 출력: result_log/log/ensemble_gem_level2-7_test{N}/
"""
...
def load_models(device: str = "cpu"):
    """모델 로드 with custom GeM Pooling (p=3)"""
    print(f"Loading models with custom GeM Pooling on device: {device}")

    # 기본 모델 로드 (global_pool 제거)
    vit = timm.create_model("vit_small_patch16_224", pretrained=True, num_classes=0, global_pool="").eval().to(device)
    conv = timm.create_model("convnext_large.fb_in22k_ft_in1k_384", pretrained=True, num_classes=0, global_pool="").eval().to(device)

    # GeM Pooling 레이어 추가
    gem_pool = GeM(p=3).to(device)

    return vit, conv, gem_pool


def embed_with_gem(img: Image.Image, model, transform, gem_pool) -> np.ndarray:
    """GeM Pooling을 사용한 임베딩 추출"""
    if img.mode != "RGB":
        img = img.convert("RGB")
    img = enhance_image(img)
    x = transform(img).unsqueeze(0)
    with torch.no_grad():
        features = model(x)  # raw features without pooling
        z = gem_pool(features).cpu().numpy()[0]  # apply GeM pooling
...
def predict_both_gem(pc: Pinecone, idx_conv, idx_vit, img_path: Path, vit, conv, tf_v, tf_c, gem_pool) -> Tuple[np.ndarray, np.ndarray]:
    """GeM Pooling을 사용한 예측"""
    img = Image.open(img_path)
    vq = embed_with_gem(img, vit, tf_v, gem_pool)
    cq = embed_with_gem(img, conv, tf_c, gem_pool)
...
def main():
    ...
    device = "cpu"
    vit, conv, gem_pool = load_models(device)
    tf_v, tf_c = tf_vit(), tf_conv()

    # 로그 디렉토리 구성
    prefix = "ensemble_gem_level2-7_test"
    ...
    for fp, st in test_items:
        try:
            p_c, p_v = predict_both_gem(pc, idx_conv, idx_vit, fp, vit, conv, tf_v, tf_c, gem_pool)
    ...
    # 리포트 저장 (GeM Pooling 버전)
    with open(log_dir/"report.txt", "w", encoding="utf-8") as f:
        f.write("="*80+"\n")
        f.write(f"ConvNeXt+ViT Ensemble with GeM Pooling Test (Level 2-7) - Test #{test_no}\n")
        f.write("="*80+"\n")
        ...
        f.write(f"풀링 방식: GeM Pooling (p=3) - 탈모 특징 강조\n\n")
    ...
    plt.title(f'ConvNeXt+ViT Ensemble with GeM Pooling Confusion Matrix (Level 2-7)\nTest #{test_no}')
    ...
    plt.title(f'ConvNeXt+ViT Ensemble with GeM Pooling Performance (Level 2-7) - Test #{test_no}')
    plt.tight_layout()
    ...
    with open(cfg_out, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "pooling_method": "gem",
            "gem_p": 3,
            "pooling_note": "GeM Pooling for enhanced hair loss feature detection",
            "config": cfg,
