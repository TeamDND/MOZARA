#!/usr/bin/env python3
r"""
ConvNeXt + ViT-S/16 듀얼 이미지 Middle Fusion 앙상블 테스터
모델별로 두 이미지를 먼저 융합한 후 앙상블 적용

테스트 대상: C:\Users\301\Desktop\top_down_side_test
- 파일명 첫 숫자가 같으면 동일인
- Top-down or Front + Right or Left 각 1장씩

로그 출력: result_log/log/middle_fusion_test{N}/
"""
...
def get_adaptive_weights(primary_view: str, secondary_view: str) -> Tuple[float, float]:
    """뷰포인트 조합에 따른 적응적 가중치 결정"""
    # ConvNeXt weight, ViT weight for each viewpoint
    if primary_view == 'top-down':
        # Top-down: ConvNeXt가 지역적 패턴에 강함
        conv_primary_weight = 0.6
        vit_primary_weight = 0.4
    elif primary_view == 'front':
        # Front: ViT가 글로벌 헤어라인 패턴에 강함
        conv_primary_weight = 0.4
        vit_primary_weight = 0.6
    else:
        conv_primary_weight = 0.5
        vit_primary_weight = 0.5

    if secondary_view in ['left', 'right']:
        # Side views: ConvNeXt가 측면 패턴 감지에 좋음
        conv_secondary_weight = 0.6
        vit_secondary_weight = 0.4
    else:
        conv_secondary_weight = 0.5
        vit_secondary_weight = 0.5

    return (conv_primary_weight, vit_primary_weight,
            conv_secondary_weight, vit_secondary_weight)
...
def middle_fusion_predict(p_conv1: np.ndarray, p_vit1: np.ndarray,
                         p_conv2: np.ndarray, p_vit2: np.ndarray,
                         cfg: Dict, primary_view: str, secondary_view: str,
                         use_adaptive_weights: bool = True) -> Tuple[int, np.ndarray, Dict]:
    """Middle Fusion으로 두 이미지 결합"""

    if use_adaptive_weights:
        # 적응적 가중치 사용
        conv_p_w, vit_p_w, conv_s_w, vit_s_w = get_adaptive_weights(primary_view, secondary_view)
    else:
        # 고정 가중치 사용
        conv_p_w = vit_p_w = conv_s_w = vit_s_w = 0.5

    # 모델별로 두 이미지의 확률분포 융합
    p_conv_fused = conv_p_w * p_conv1 + (1 - conv_p_w) * p_conv2
    p_vit_fused = vit_p_w * p_vit1 + (1 - vit_p_w) * p_vit2

    # 정규화
    p_conv_fused = p_conv_fused / (p_conv_fused.sum() + 1e-12)
    p_vit_fused = p_vit_fused / (p_vit_fused.sum() + 1e-12)

    # 융합된 확률에 앙상블 적용
    pred, p_final = apply_ensemble(p_conv_fused, p_vit_fused, cfg)

    # 가중치 정보 반환
    fusion_info = {
        'conv_primary_weight': conv_p_w,
        'vit_primary_weight': vit_p_w,
        'conv_secondary_weight': 1 - conv_p_w,
        'vit_secondary_weight': 1 - vit_p_w,
        'use_adaptive': use_adaptive_weights
    }

    return pred, p_final, fusion_info
...
def main():
    ...
    print(f"\n총 {len(image_pairs)}개 듀얼 이미지 페어에서 Middle Fusion 테스트 시작")
    ...
    for pair in image_pairs:
        try:
            ...
            # Middle Fusion (적응적 가중치 사용)
            pred, p_fused, fusion_info = middle_fusion_predict(
                p_c1, p_v1, p_c2, p_v2, cfg,
                pair['primary_image']['viewpoint'],
                pair['secondary_image']['viewpoint'],
            ...
            print(f"    Conv 가중치: {fusion_info['conv_primary_weight']:.2f}/{fusion_info['conv_secondary_weight']:.2f}")
            print(f"    ViT 가중치: {fusion_info['vit_primary_weight']:.2f}/{fusion_info['vit_secondary_weight']:.2f}")

        except Exception as e:
            print(f"[skip pair] {pair['person_key']}: {e}")

    dt = time.time()-t0
    print(f"\nMiddle Fusion inference done on {len(y_true)} pairs in {dt:.1f}s")
    ...
    with open(log_dir/"report.txt", "w", encoding="utf-8") as f:
        f.write("="*80+"\n")
        f.write(f"ConvNeXt+ViT 듀얼 이미지 Middle Fusion 테스트 - Test #{test_no}\n")
        f.write("="*80+"\n")
        f.write(f"테스트 일시: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"테스트 데이터: {TEST_ROOT}\n")
        f.write(f"융합 방식: Middle Fusion (적응적 가중치)\n")
        ...
        # 개별 인원별 상세 결과 (Middle Fusion 가중치 포함)
        f.write("👤 인원별 상세 결과 (Middle Fusion 가중치)\n")
        f.write("-"*40+"\n")
        for i, result in enumerate(results, 1):
            ...
            # Middle Fusion 가중치 정보
            fi = result['fusion_info']
            f.write(f"    Fusion 가중치:\n")
            f.write(f"      ConvNeXt: {fi['conv_primary_weight']:.2f}({result['primary_viewpoint']}) + {fi['conv_secondary_weight']:.2f}({result['secondary_viewpoint']})\n")
            f.write(f"      ViT: {fi['vit_primary_weight']:.2f}({result['primary_viewpoint']}) + {fi['vit_secondary_weight']:.2f}({result['secondary_viewpoint']})\n")
            f.write(f"      적응적 가중치: {fi['use_adaptive']}\n")
        ...
        if deviation_analysis['total_large_deviations'] > 0:
            ...
            for i, case in enumerate(deviation_analysis['large_deviations'], 1):
                ...
                fi = case.get('fusion_info', {})
                if fi:
                    f.write(f"      가중치 - Conv: {fi.get('conv_primary_weight', 0.5):.2f}/{fi.get('conv_secondary_weight', 0.5):.2f}, ")
                    f.write(f"ViT: {fi.get('vit_primary_weight', 0.5):.2f}/{fi.get('vit_secondary_weight', 0.5):.2f}\n")

            f.write("\n🔍 Middle Fusion 큰 편차 발생 원인 분석:\n")
            f.write("  - 적응적 가중치의 한계: 개별 이미지 품질 차이 미반영\n")
            f.write("  - 모델별 융합 효과: ConvNeXt/ViT의 뷰포인트별 특성 차이\n")
            f.write("  - 앙상블 단계 영향: 융합된 확률에 per-class 가중치 적용\n")
            f.write("  - 뷰포인트 조합 최적화 필요: 특정 조합에서 가중치 조정 요구\n")
        f.write("\n")
        ...
        # Middle Fusion 가중치 전략 요약
        f.write("🔄 Middle Fusion 가중치 전략\n")
        f.write("-"*40+"\n")
        f.write("적응적 가중치 규칙:\n")
        f.write("  Top-down Primary: ConvNeXt 0.6, ViT 0.4 (지역적 패턴 강화)\n")
        f.write("  Front Primary: ConvNeXt 0.4, ViT 0.6 (글로벌 헤어라인 강화)\n")
        f.write("  Left/Right Secondary: ConvNeXt 0.6, ViT 0.4 (측면 패턴 강화)\n")
        f.write("  기타: ConvNeXt 0.5, ViT 0.5 (균등 가중치)\n\n")
    ...
    plt.title(f'Middle Fusion Confusion Matrix (Level 1-7)\nTest #{test_no}')
    ...
    plt.title(f'Middle Fusion 4단계 노우드 분류 Confusion Matrix\nTest #{test_no}')
    ...
    plt.title(f'Middle Fusion 성능 - Test #{test_no}')
    plt.tight_layout()
    plt.savefig(log_dir/"performance_metrics.png", dpi=300, bbox_inches='tight')
    plt.close()

    # 설정 파일 저장
    cfg_out = log_dir / "middle_fusion_config.json"
    with open(cfg_out, "w", encoding="utf-8") as f:
        json.dump({
            "test_type": "dual_image_middle_fusion",
            "adaptive_weights": True,
            "weight_strategy": {
                "top-down_primary": {"conv": 0.6, "vit": 0.4},
                "front_primary": {"conv": 0.4, "vit": 0.6},
                "side_secondary": {"conv": 0.6, "vit": 0.4},
                "default": {"conv": 0.5, "vit": 0.5}
            },
    ...
    # 간단한 성능 요약 출력
    print(f"\n{'='*60}")
    print(f"Middle Fusion Test #{test_no} 완료!")
    print(f"{'='*60}")
    print(f"정확도: {acc:.3f} | F1-Score: {f1:.3f}")
    print(f"4단계 노우드 정확도: {norwood_4stage_metrics['accuracy']:.3f}")
