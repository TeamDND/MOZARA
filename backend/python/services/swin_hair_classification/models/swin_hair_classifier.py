...
class PatchEmbed(nn.Module):
    r""" Image to Patch Embedding for dual input (RGB + Mask)
    """
...
class SwinHairClassifier(nn.Module):
    r""" Swin Transformer for Hair Loss Classification
...
def swin_tiny_patch4_window7_224_hair(num_classes=4, **kwargs):
    """Swin-T for hair loss classification"""
...
def swin_small_patch4_window7_224_hair(num_classes=4, **kwargs):
    """Swin-S for hair loss classification"""
    model = SwinHairClassifier(
        patch_size=4,
        window_size=7,
        embed_dim=96,
...
if __name__ == "__main__":
    ...
    # Test with dual input (RGB + Mask)
    batch_size = 2
    rgb_image = torch.randn(batch_size, 3, 224, 224)  # Original image
    mask_image = torch.randn(batch_size, 3, 224, 224)  # Mask image

    # Concatenate to 6-channel input
    dual_input = torch.cat([rgb_image, mask_image], dim=1)  # (B, 6, H, W)
