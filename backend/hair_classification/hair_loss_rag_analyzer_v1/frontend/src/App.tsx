...
import { analyzeImages, AnalysisResult as AnalysisResultData } from './services/api';
...
function App() {
  ...
  const handleAnalyzeClick = async () => {
    ...
    const result = await analyzeImages(primaryImage, secondaryImage);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800">
      <header className="bg-white shadow-sm">
        <div className="max-w-4xl mx-auto py-4 px-4">
          <h1 className="text-3xl font-bold text-gray-900">탈모 유형 분석기</h1>
        </div>
      </header>
      <main className="max-w-4xl mx-auto py-8 px-4">
        <div className="text-center">
            <p className="mt-2 text-lg text-gray-600">정수리(Top-down) 사진과 측면(Side) 사진을 업로드하여 탈모 단계를 분석합니다.</p>
        </div>
        
        <ImageUpload 
          onPrimaryImageSelect={setPrimaryImage}
          onSecondaryImageSelect={setSecondaryImage}
        />

        <div className="text-center">
            <button 
              onClick={handleAnalyzeClick} 
              disabled={isLoading || !primaryImage || !secondaryImage}
