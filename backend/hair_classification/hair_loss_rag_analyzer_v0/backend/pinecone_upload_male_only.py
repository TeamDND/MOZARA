#!/usr/bin/env python3
"""남자 데이터만 파인콘 업로드 (API 레이트 리미트 대응)"""
...
class MalePineconeUploader:
    """남자 데이터만 업로드하는 파인콘 매니저"""
    ...
    def get_male_images_only(self, dataset_path: str) -> List[str]:
        """남자 이미지 파일만 찾기"""
        male_images = []
        supported_extensions = ('.jpg', '.jpeg', '.png', '.bmp', '.tiff')

        print(f"남자 이미지 파일 검색 중: {dataset_path}")

        for root, dirs, files in os.walk(dataset_path):
            # 폴더명에 'male'이 포함된 경우만 처리
        ...
        print(f"남자 이미지 총 개수: {len(male_images)}")
        return male_images

    def check_existing_vectors(self) -> set:
        """이미 업로드된 벡터 ID들 확인"""
        try:
            # 현재 업로드된 벡터 중 남자 데이터 확인
            existing_ids = set()
            ...
            for match in result['matches']:
                if match['id'].startswith('male_'):
                    existing_ids.add(match['metadata'].get('filename', ''))

            print(f"이미 업로드된 남자 이미지: {len(existing_ids)}개")
            return existing_ids
    ...
    def upload_male_data_with_retry(self, dataset_path: str) -> bool:
        """재시도 로직이 포함된 남자 데이터 업로드"""
        try:
            # 1. 남자 이미지만 가져오기
            male_images = self.get_male_images_only(dataset_path)

            if not male_images:
                print("남자 이미지를 찾을 수 없습니다.")
                return False
            ...
            print(f"새로 업로드할 남자 이미지: {len(images_to_upload)}개")
            ...
            print(f"남자 데이터 업로드 완료!")
            print(f"성공: {success_count}개, 실패: {error_count}개")
            return True

        except Exception as e:
            self.logger.error(f"남자 데이터 업로드 실패: {e}")
            return False
...
def main():
    ...
    try:
        # 업로더 초기화
        uploader = MalePineconeUploader(api_key=api_key)
        ...
        if uploader.upload_male_data_with_retry(dataset_path):
            print("남자 데이터 업로드 성공!")
