...
public interface UserMetricsRepository extends JpaRepository<UserMetricEntity, Integer> {
    ...
    // 사용자별 메트릭 조회
    List<UserMetricEntity> findByUserIdForeign_UserIdOrderByRecordDateDesc(Long userId);
    ...
    // 사용자별 특정 타입 메트릭 조회
    @Query("SELECT m FROM UserMetricEntity m WHERE m.userIdForeign.userId = :userId AND m.type = :type ORDER BY m.recordDate DESC")
    List<UserMetricEntity> findByUserAndType(@Param("userId") Long userId, @Param("type") String type);
