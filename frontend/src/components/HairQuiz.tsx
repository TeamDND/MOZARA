...
const HairQuiz: React.FC = () => {
  ...
  const generateQuizWithGemini = async (): Promise<QuizQuestion[]> => {
    ...
    const prompt = `당신은 탈모 전문가이자 재치있는 퀴즈 출제자입니다. 탈모와 관련된 전문적 지식과 재미있는 상식을 섞어서, O/X 퀴즈 20개를 생성해주세요. 답변(answer)은 반드시 'O' 또는 'X'로만 해주세요. 결과를 반드시 다음의 JSON 배열 형식으로만 응답해주세요. 다른 설명은 일절 추가하지 마세요. [{"question": "<질문>", "answer": "<O 또는 X>", "explanation": "<정답에 대한 상세하고 친절한 설명>"}, ...20개]`;
